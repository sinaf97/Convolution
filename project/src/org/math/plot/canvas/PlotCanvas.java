...
public abstract class PlotCanvas extends JPanel implements MouseListener, MouseMotionListener, ComponentListener, BaseDependant, MouseWheelListener {
    ...
    public final static int DRAGABLE = 2;
    ...
    int x_axis = -15;
    int y_axis = 150;
    LinkedHashMap<Double,Double> result;
    public LinkedHashMap<Double,Double> mouseMovement = new LinkedHashMap<>();
    public LinkedHashMap<Double,Double> mouseMovement2 = new LinkedHashMap<>();
...
//                update(mouseMovement);
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        switch (ActionMode) {
            ...
            case ZOOM:
                ...
                break;
            case DRAGABLE:
                if(Math.abs(mousePrev[0]-mouseCurent[0])>5) {
                    if (mousePrev[0] < mouseCurent[0]) {
                        this.mouseMovement = shift(this.mouseMovement, 1);
                        this.mouseMovement2 = shift(this.mouseMovement2, -1);
                    }
                    else {
                        this.mouseMovement = shift(this.mouseMovement, -1);
                        this.mouseMovement2 = shift(this.mouseMovement2, 1);
    ...
    public LinkedHashMap<Double,Double> shift(LinkedHashMap<Double,Double> x,int shift){
        LinkedHashMap<Double,Double> result = new LinkedHashMap<>();
        for (Map.Entry<Double, Double> entry : x.entrySet())
            result.put(entry.getKey()+shift/scale*2,entry.getValue());
        return result;
...
//                update(mouseMovement);
    ...
    public void update(LinkedHashMap<Double,Double> mouseMovement){
        double[] min = {-10,-10};
        double[] max = {10,10};
